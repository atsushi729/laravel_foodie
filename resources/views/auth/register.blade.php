        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('register') }}" class="p-5">
                ...
                <div class="form-group">
                    ...
                    <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus placeholder="名前を入力して下さい">
                    @error('name')
                ...
                <div class="mt-2">
                    ゲストユーザーとしてのご利用は<a href="{{ route('login') }}">ログイン画面へ</a>
                </div>
            </form>
        </div>
    </div>
